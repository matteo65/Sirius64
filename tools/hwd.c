...
/* 
   ...
   David Blackman and Sebastiano Vigna. A new test for Hamming-weight dependencies.
   ACM Trans. Model. Comput. Simul., 32(3):1−19, 2022.
   ...
   - Optionally, HWD_NUMCATS, if you want to override the default number
     ...
     is HWD_DIM/2 + 1.
   - Optionally, HWD_MMAP if you want to allocate memory using transparent
     huge pages via mmap()/madvise().
...
int main(int argc, char **argv) {
	...
	uint64_t mem = (SIZE + 1) * sizeof *cs + SIZE * sizeof *norm + SIZE * sizeof *count_sum;
	cs = mmap((void *)(0x0UL), mem, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, 0, 0);
	if (cs == MAP_FAILED) {
		fprintf(stderr, "Failed.\n");
		exit(1);
	}
	fprintf(stderr, "OK.\n");
	madvise(cs, mem, MADV_HUGEPAGE);
